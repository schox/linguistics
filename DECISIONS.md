...
## 2026-08-13: The Human Languages selection is popularity plus genealogical coverage, and WALS is deferred

**Decided.** Which individual languages earn a note is settled on Andrew's criterion, the same one applied to Computer Languages the day before: the most spoken languages, plus one or two from each family. 51 languages across 24 families, added as stubs. WALS, this area's candidate instrument since the taxonomy was cut, is not the selection instrument and is deferred to the typology work it was built for.

**Why.** [WALS 2013](references/wals-2013.md) is the better answer on its own terms, and it answers a different question. Its sample is constructed to "maximize both genealogical and areal diversity" for typological survey, which means it is indifferent to prominence by design: it cannot supply the popularity half of the criterion at all, and it omits the classical corpus languages the Decipherment area turns on, so those would have had to be added on a separate stated basis anyway. The practical obstacle was secondary and is recorded because it will recur: the sample list is served through a JavaScript table rather than as a file, and the bulk download URL tried on 2026-08-12 returned 404, so adopting it meant reconstructing the list by hand from a source that could not be cited as retrieved. Using one criterion across both language areas also means the two are comparable, and the vault has one selection principle to defend rather than two.

**What the popularity half actually rests on.** Four languages. [Ethnologue 2026](references/ethnologue-2026.md) is the standard ranking and sells its ranked list; the free pages state the top four in prose, and both pages had to be retrieved through the Tavily extraction service after `ethnologue.com` returned 403. So Mandarin Chinese, Spanish, English and Hindi are selected on sourced speaker numbers and the other 47 are selected for genealogical coverage. Each stub says which it is, and the reference note carries the macrolanguage caveat that decides the naming: Ethnologue ranks individual languages rather than macrolanguages, which is why the selection holds Mandarin Chinese and Modern Standard Arabic and not "Chinese" and "Arabic".

**Consequence.** The taxonomy program has no open selection questions left. The coverage half is a judgement, inherited from this area's own index sections and from no external instrument, and it is labeled as one in every note it produced rather than presented as sourced. Purchasing the Ethnologue list, or reaching it through an institution, is now a concrete unblocking action rather than a vague want. Nine isolates were retyped from `Concept` to `Language` in the same change, since for an isolate the family and the language are the same thing and typing them as groupings misdescribed them.
