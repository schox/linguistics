...
- **The `class` enum, now that the typology is written.** Writing `script-typology.md` surfaced three mismatches between the enum and the sources, and all three are judgement calls rather than lookups.
  - **`featural` is missing.** Both Baroni and the Unicode Standard carry the category, for Hangul. Nothing in the corpus needs it today, and adding an enum value for a script with no note is speculative.
  - **`logographic` against `logosyllabary`.** The enum says the first; Unicode uses the second and reserves it for systems that also write sound, which is all the attested ones. Renaming touches every note carrying the value.
  - **`mixed` and `undetermined` are local inventions.** Neither source uses either as a category name. Unicode covers the first case with "composite system" and the second by simply not classifying what it does not encode. Six notes carry `mixed` and four carry `undetermined`, so both are load-bearing here, but they are not standard vocabulary and the notes should say so if they are kept.
...
3. **General Linguistics, Writing systems.** ~~The most urgent thing in the vault.~~ **Anchored 2026-08-12** by `General-Linguistics/script-typology.md`, which defines the vocabulary the `class` field draws on, from [Baroni 2011](references/baroni-2011.md), [the Unicode Standard](references/unicode-standard-17.md) chapter 6 with its Table 6-1, and the numeric ranges of [ISO 15924](references/iso15924.md). The three do not fully agree, and the note says where.
   ...
   **Two pieces of follow-up work it generated**, both mechanical rather than research:
   ...
   - **The `class` enum needs a decision**, listed under Blocked on Andrew below.
