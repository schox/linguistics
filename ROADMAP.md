...
What phase 2 produced, beyond the notes, is a second axis for the whole area. Phase 1 established that corpus size is the binding constraint on the unread scripts. Phase 2 shows that among the *solved* ones the decisive variables were different every time: a wrong theory for Egyptian, an argument from implausibility for cuneiform, institutional authority and Cold War politics for Maya, and for Ugaritic neither, just thirty signs and a correct guess at the language family. Those belong in a junction note. One batch each, each on *what actually broke it*, which is usually a bilingual, a proper name, or a structural regularity rather than cleverness. Older sources, far more likely to be open access than phase 1's were. Ugarit is needed as a `Place` note here and is already overdue as the sole provenance of the CM3 texts.
