...
**The obvious next use of that note is to apply it.** None of the seven unread scripts has been diagnosed against it. The Voynich manuscript is the interesting case, because it has abundant text and therefore cannot be in the evidential family at all. One batch each, each on *what actually broke it*, which is usually a bilingual, a proper name, or a structural regularity rather than cleverness. Older sources, far more likely to be open access than phase 1's were. Ugarit is needed as a `Place` note here and is already overdue as the sole provenance of the CM3 texts.
