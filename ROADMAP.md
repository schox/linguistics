...
7. **Computer Languages.** Stubbed out on 2026-08-12: 49 topic stubs across all eight subfields. `DECISIONS.md` previously recommended leaving this area thin, a recommendation made under argument-first batching that did not survive the change to breadth-first.
   ...
   **The language layer is blocked on the same selection principle as Human Languages**, and the two should be settled together. The lineages are stubbed as the spine, but no individual language has a note beyond [Lisp](Computer-Languages/lisp.md): no C, no Python, no Haskell. [HOPL](references/hopl.md) is held and unused and is the candidate instrument, as WALS is for human languages. Both are open decisions and neither should be settled by picking favorites.
