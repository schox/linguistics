---
type: MOC
aliases:
  - "Epistemics"
area: Decipherment
subfield:
  - Epistemics
belongs_to: "[[Decipherment]]"
updated: 2026-08-12
---
# Epistemics

What counts as a decipherment.

**Stub hub.** This note exists so the taxonomy is visible in the graph before the subfield has content; the breadth program in `ROADMAP.md` fills it in. Notes in this subfield set `belongs_to` here once it has an owner below the area hub.

## Topics owed to this hub

Evidentiary thresholds, falsification criteria, overfitting and pareidolia, why a model always outputs something, why undeciphered scripts attract cranks, whether a given corpus is writing at all.

## Notes in this subfield

- [Adversarial versus accidental encipherment](adversarial-versus-accidental.md)
- [Cuneiform](cuneiform.md)
- [The evidentiary threshold and unicity distance](evidentiary-threshold-and-unicity-distance.md)
- [Indus script](indus-script.md)
- [Is it writing at all? The Indus entropy debate](is-it-writing-at-all.md)
- [Mayan hieroglyphs](maya-script.md)
- [Phaistos Disc](phaistos-disc.md)
- [Rongorongo](rongorongo.md)
- [What counts as one sign](segmentation-and-transcription.md)
- [Voynich manuscript](voynich-manuscript.md)
- [Why a script stays unread](why-scripts-stay-unread.md)
