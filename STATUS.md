...
The vault currently holds 335 notes, of which 51 are subfield hub stubs and 125 are topic stubs so the whole taxonomy is visible in the graph. Decipherment is no longer a skeleton; every other area's content still is, though its shape now shows.
...
- **Cryptography**: 3 written notes and 125 topic stubs, across 146 files. Six of fourteen subfields are stubbed out in full: Cryptanalysis, Classical ciphers, Symmetric cryptography, Public-key cryptography, Digital signatures and authentication, Hashes and integrity. **This area is now the largest in the corpus by file count and has three written notes in it.** The subfield vocabulary was rebuilt on 2026-08-12 against three published classifications (see the area index); 14 subfields, of which **Cryptanalysis** is the first to be stubbed out in full. The other 13 hold no content note at all.
