...
The vault currently holds 657 notes, of which 7 are subfield hub stubs and 446 are topic stubs so the whole taxonomy is visible in the graph. Decipherment is no longer a skeleton; every other area's content still is, though its shape now shows.
...
- **Computer Languages**: 1 written note (Lisp) and 49 topic stubs, across 58 files. All eight subfields stubbed 2026-08-12. Individual languages are deliberately not stubbed; the lineages are the spine and the selection question is shared with Human Languages.
